...
<template>
  ...
  >
    ...
    <el-table-column :label="$t('robot.projectId')" align="center">
      <template #default="scope"
        ><el-tag v-if="scope.row.projectId == null" type="info">{{
          $t('robot.consts.anyProject')
        }}</el-tag
        ><template v-else>{{
          projectMap[scope.row.projectId].projectName
        }}</template>
      </template>
    </el-table-column>
    ...
    <el-table-column :label="$t('robot.muteRule')" align="center"
      ><template #default="scope">
        ...
        <el-popover v-else trigger="hover" width="auto">
          ...
          <template #reference>
            <el-tag>{{ $t('robot.consts.customMute') }}</el-tag>
          </template>
        </el-popover>
      </template>
    </el-table-column>
