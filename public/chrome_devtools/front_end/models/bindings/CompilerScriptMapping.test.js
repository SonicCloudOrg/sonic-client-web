...
describeWithMockConnection('CompilerScriptMapping', () => {
    ...
    it('removes webpack hashes from display names', async () => {
        ...
        const scriptInfo = { url: `${sourceRoot}/bundle.js`, content: '1;\n' };
        const sourceMapInfo = { url: `${scriptInfo.url}.map`, content: { version: 3, mappings: '', sourceRoot, sources } };
        const namesPromise = Promise.all(sources.map(name => waitForUISourceCodeAdded(`${sourceRoot}/${name}`, target).then(uiSourceCode => uiSourceCode.name())));
    ...
    it('creates separate UISourceCodes for separate targets', async () => {
        ...
        for (const { script, uiSourceCode } of [{ script: mainScript, uiSourceCode: mainUISourceCode },
    ...
    it('creates separate UISourceCodes for content scripts', async () => {
        ...
        for (const { script, uiSourceCode } of [{ script: regularScript, uiSourceCode: regularUISourceCode },
    ...
    describe('supports modern Web development workflows', () => {
        it('supports webpack code splitting', async () => {
            ...
            const route1Locations = await debuggerWorkspaceBinding.uiLocationToRawLocations(route1UILocation.uiSourceCode, route1UILocation.lineNumber, route1UILocation.columnNumber);
            ...
            const route2Locations = await debuggerWorkspaceBinding.uiLocationToRawLocations(route2UILocation.uiSourceCode, route2UILocation.lineNumber, route2UILocation.columnNumber);
            ...
            const sharedLocations = await debuggerWorkspaceBinding.uiLocationToRawLocations(sharedUILocation.uiSourceCode, sharedUILocation.lineNumber, sharedUILocation.columnNumber);
        ...
        it('supports webpack hot module replacement', async () => {
            ...
            assert.deepEqual(await debuggerWorkspaceBinding.rawLocationToUILocation(originalScript.debuggerModel.createRawLocation(originalScript, 0, 0)), originalLibUISourceCode.uiLocation(0, 0));
            assert.deepEqual(await debuggerWorkspaceBinding.rawLocationToUILocation(originalScript.debuggerModel.createRawLocation(originalScript, 1, 0)), originalAppUISourceCode.uiLocation(0, 0));
            ...
            const { uiSourceCode } = (await debuggerWorkspaceBinding.rawLocationToUILocation(originalScript.debuggerModel.createRawLocation(originalScript, 1, 0)));
