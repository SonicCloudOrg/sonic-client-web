...
describeWithEnvironment('PageLoadMetricsHandler', function () {
    describe('contentful paints', () => {
        it('obtains all the FCP and LCP events for all frames', async function () {
            ...
            const fcpCount = countMetricOcurrences(scoresByMetricName, "FCP" /* TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FCP */);
            const lcpCount = countMetricOcurrences(scoresByMetricName, "LCP" /* TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.LCP */);
    ...
    describe('markDOMContent frame', () => {
        it('obtains them and assigns them to the correct frames', async function () {
            ...
            // and one for the second.
            assert.strictEqual(pageLoadEventsForMainFrame.size, 2);
            const dclCount = countMetricOcurrences([...pageLoadEventsForMainFrame.values()], "DCL" /* TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.DCL */);
    ...
    describe('metric scores', () => {
        ...
        function assertMetricNavigationId(metric, navigationId) {
        ...
        it('extracts DOMContentLoaded correctly', () => {
            const domContentLoadedMetrics = getMetricsByName("DCL" /* TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.DCL */);
        ...
        it('extracts First Contentful Paint correctly', () => {
            const firstContentfulPaints = getMetricsByName("FCP" /* TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FCP */);
        ...
        it('extracts Largest Contentful Paint correctly', () => {
            const firstContentfulPaints = getMetricsByName("LCP" /* TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.LCP */);
        ...
        it('extracts First Paint correctly', () => {
            const firstContentfulPaints = getMetricsByName("FP" /* TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FP */);
            assert.strictEqual(firstContentfulPaints[0].timing, 37269);
        ...
        it('extracts Load correctly', () => {
            const firstContentfulPaints = getMetricsByName("L" /* TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.L */);
    ...
    describe('Marker events', () => {
        ...
        it('extracts all marker events from a trace correctly', () => {
            for (const metricName of TraceModel.Types.TraceEvents.MarkerName) {
                ...
                assert.isTrue(markerEventsOfThisType.every(marker => TraceModel.Handlers.ModelHandlers.PageLoadMetrics.getFrameIdForPageLoadEvent(marker) === mainFrameId));
            }
        });
        ...
        it('only stores the largest contentful paint with the highest candidate index', async function () {
            ...
            const largestContentfulPaints = pageLoadMarkers.filter(TraceModel.Types.TraceEvents.isTraceEventLargestContentfulPaintCandidate);
